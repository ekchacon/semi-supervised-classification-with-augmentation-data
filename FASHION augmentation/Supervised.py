...
import numpy as np
import math
from mypackages.orderSeq import order as order
...
for j in range(2,20):# tailor the amount of labeled training example to be used from 0.33% to 16.67%.
  ...
  for i in range(0,10,1):
    ...
    model.fit(train,epochs=1000, 
                          ...
                          validation_steps=19, #9 for 10k (test data) entre 1024 (batchsize)
                          callbacks=cbks
              )
  ...
  f = open('/home/edgar/Edgar/Semestre 5/model implementation/fashion augmentation/supervised/experiment0/log/supervised.csv','a')
